    ...
    useEffect(() => {
      const getEstablishmentDetails = async () => {
        ...
        if (establishmentData?.data?.success) {
          ...
          setValue('salonName', establishmentData?.data?.data?.profile?.establishmentName)
    ...
    return (
        ...
        <section className='w-full flex justify-center items-center flex-1 overflow-y-auto pt-64' style={{ height: "80vh" }}>
            <div style={{ width: "50%", padding: "0px 20px" }}>
                ...
                <h4 className="text-xl md:text-3xl tracking-wide mb-2 font-bold">Tell us about your business</h4>
                <h4 className="text-base md:text-base tracking-wide mb-10 ml-2">Enter the Salon name and location</h4>
                <TextField 
                    {...register("salonName")}
                    id="outlined-basic" 
                    size="medium" 
             ...
             <Button variant="text" size='large' sx={{textTransform:"none", color: '#825FFF', fontWeight: 'bold', fontSize: '18px'}} 
