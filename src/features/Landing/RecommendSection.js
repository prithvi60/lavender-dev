...
const RecommendSection = () => {
  ...
  const NextArrow = (props) => {
    ...
    return <KeyboardArrowRightOutlined className={`${className} arrow next-arrow`} onClick={onClick} />;
  };

  const PrevArrow = (props) => {
    const { className, onClick } = props;
    return <KeyboardArrowLeftOutlined className={`${className} arrow prev-arrow`} onClick={onClick} />;
  };
  ...
  useEffect(() => {
    const imageIds = establishmentSearchResult?.data?.data?.content?.map(
      (establishment) => (establishment.estImages.length > 0 ? establishment.estImages[0] : '')
    );
                ...
                <CardContent className="card-content">
                  ...
                  <div className="card-tags gap-1" sx={{ display: 'flex', justifyContent: 'center' }}>
                    {card?.serviceTags?.length > 0 ? (
                      card?.serviceTags?.map((tag, index) => <Chip key={index} label={tag} className="small" />)
                    ) : (
