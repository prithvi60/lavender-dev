...
const AddressForm = () => {
  ...
  const FORM_VALUES = [
    ...
    {xs: 12, sm: 12, type: "textArea", id: "about", label: "About", autoComplete: "about", variant: "standard", required: false, placeholder: "Input the establishment description"},
      ...
      <Grid container spacing={3}>
        {FORM_VALUES?.map((item, index) => {
          ...
          switch (item?.type) {
            case "textField":
              return (
                <Grid key={index} item xs={item?.xs} sm={item?.sm}>
                  <TextField
