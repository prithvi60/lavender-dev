import { Modal, Card, Grid, Divider, Box, IconButton, Typography, Button, Slide, useMediaQuery } from '@mui/material';
...
export const BookingInfoModalListView = ({ isModalOpen, bookings, toggleModal, userflow }) => {
                    ...
                    <Box sx={contentStyle}>
                        ...
                        <Box sx={{ p: 3, bgcolor: '#EEEEFF' }}>
                            <Typography variant="h6" component="h2" gutterBottom>
                                {convertToDateOnly(bookings?.startTime)} {convertToMonthOnly(bookings?.startTime)}, {convertToYearOnly(bookings?.startTime)}
                            </Typography>
                        ...
                        <Typography variant="h6" gutterBottom sx={{ p: 2, color: '#825FFF' }}>
                        ...
                        <Box sx={{ p: 3, flexGrow: 1, overflowY: 'auto' }}>
                            {bookings?.services?.map((service, index) => (
                                <Box key={index} sx={{ mb: 2 }}>
                                    ...
                                    <Typography variant="body2">{service?.bookingStatus}</Typography>
                                </Box>
                            ))}
                            <Box sx={{ mt: 2 }}>
                            ...
                            <Box sx={{ mt: 2 }}>
                                <Typography variant="subtitle1">Cancellation policy at this venue</Typography>
                                <Typography variant="body2">
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'space-around', p: 2 }}>
                            <Button startIcon={<GetIcon iconName="LocationIcon" />}>
                                Directions
                            </Button>
                            {userflow !== 'past' && (
                                <>
